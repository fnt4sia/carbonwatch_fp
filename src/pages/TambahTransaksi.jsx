...
const calculateSuddenSpike = async (companyId, currentAmount, currentHour) => {
  ...
  const { data: recentTransactions, error } = await supabase
    .from('transaction')
    .select('transaction_amount, transaction_hour')
    .eq('company_id', companyId)
    .order('transaction_hour', { ascending: false })
  ...
  const amounts = recentTransactions.map(t => t.transaction_amount);
  ...
  const hourDiffs = recentTransactions.map(t => Math.abs(t.transaction_hour - currentHour));
...
export default function TambahTransaksi() {
  ...
  const processCSVData = async (csvData) => {
    ...
    for (const row of results.data) {
      ...
      const transactionAmount = parseFloat(row.transaction_amount);
      const transactionHour = parseInt(row.transaction_hour);
      const isSpike = await calculateSuddenSpike(
        company.company_id,
        transactionAmount,
        transactionHour
      );
      ...
      const apiData = {
        ...
        "Carbon Volume": parseFloat(row.carbon_volume),
        "Price per Ton": parseFloat(row.price_per_ton),
        "Origin Country": row.origin_country,
        "Cross-Border Flag": row.cross_border_flag.toLowerCase() === 'true',
        "Buyer Industry": row.buyer_industry,
